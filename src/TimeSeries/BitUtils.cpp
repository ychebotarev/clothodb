    ...
    uint8_t bitsToStore)
{
    ...
    {
        buffer[position] +=  (uint8_t)(value << (availableBits - bitsToStore));
        return;
    }
    ...
    uint8_t bitsToRead)
{
    ...
    {
        return ReadMidBits(buffer[position], 8 - availableBits, bitsToRead);
    }

    if (availableBits > 0)
    {
        value += ReadLoBits(buffer[position++], availableBits);
    ...
    if (bitsToRead > 0)
    {
        ...
        value += ReadHiBits(buffer[position], bitsToRead);
    }
