...
void double_compressor::append_first_value(uint64_t value)
{
    append_next_value(value);
}
...
void double_compressor::append_next_value(uint64_t value)
{
...
void integer_compressor::append_first_value(uint64_t value)
{
...
void integer_compressor::append_next_value(uint64_t value)
{
...
std::shared_ptr<bit_stream> compressor::compress_double_values(const std::vector<double>& values)
{
    ...
    compressor.append_first_value(DoubleToUint64(values[0]));
    for (size_t i = 1; i < values.size(); ++i)
    {
        compressor.append_next_value(DoubleToUint64(values[i]));
    }
    writer.commit();
    return stream;
}
