...
{
public:
    static cdb::vector_or_error<uint32_t> decompress_timestamps(bit_stream& stream);
    static cdb::vector_or_error<double> decompress_double_values(bit_stream& stream);
    static cdb::vector_or_error<uint64_t> decompress_integer_values(bit_stream& stream);

private:
    template<typename T, class D>
    static cdb::vector_or_error<T> decompress(bit_stream& stream);
};
